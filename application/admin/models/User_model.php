...
class User_model extends CI_Model
{
	...
	public function dologin($user_name,$password)
	{
		...
		if(!empty($result) && $result->adm_password == $password)
		{
			...
			//获取对应用户的权限信息
			$query_access = $this->db->query("select * from ".$this->db->dbprefix('role_access')." where role_id = ".$result->role_id);
	...
	{
		$query = $this->db->query("select * from ".$this->table." where user = ".$user);
