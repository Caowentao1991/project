          ...
          <div class="box">
            ...
            <div class="box-body"> 
              <table class="table table-bordered">
                ...
                <form method="post"  enctype"multipart/form-data" name="child_loc" action="">
                <tbody>
                ...
                <?php if(!empty($data)){foreach ($data as $k=>$v){?>
                  <tr>
                    ...
                    <td><?=$v['code'];?></td>
                    <td><?=$v['name'];?></td>
                    <td style="width: 110px;"><input type="text"  value="<?php echo $v['num_register']?>"></td>
                    <td style="width: 110px;"><input type="text"  value="<?php echo $v['num_submit']?>"></td> 
                    <td style="width: 110px;"><input type="button" value="修改" class="a"></td>
                  </tr>
                <?php }}?>
...
<script>  
  ...
  function search(){
    ...
    window.location.href = "/admin/index.php/Channel/set_num?qd_name="+qd_name;
  }

 $(function(){
    $(".a").on("click",function(){
      var qd_id = $(this).parents("tr").children('td').eq(0).text();
      var name = $(this).parents("tr").children('td').eq(2).text();
      var code = $(this).parents("tr").children('td').eq(1).text();
      var num_register = $(this).parents("tr").children('td').eq(3).children('input').val();
      var num_submit  = $(this).parents("tr").children('td').eq(4).children('input').val();
      $.ajax({
                type: "POST", //用POST方式传输
                dataType: "json", //数据格式:JSON
                url: '/admin/index.php/channel/ajax_num', //目标地址
                data: {'qd_id':qd_id,'num_register':num_register,'num_submit':num_submit,'name':name,'code':code}, 
