...
class Sms extends CI_Controller {
    ...
    public function register()
    {
        ...
        if (!empty($mobile)) {
            $one = $this->sms->get_one(['mobile'=>$mobile]);
...
//            $response['code'] = '000';
            if ($response['code'] == '000') {
                $this->sms->insert(['mobile'=>$mobile,'verifyCode'=>$verifyCode,'time'=>date('Y-m-d H:i:s')]);
                echo json_encode(array('code' => '200', 'error_code' => 0,'error_message'=>'','time'=>date('Y-m-d H:i:s'),'message'=>'OK','data'=>['sign'=>md5($mobile)]));
                die;
            } else {
