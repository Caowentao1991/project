...
//            return self::_getSae();
    ...
    /**
     * @descrpition 从微信服务器获取微信jsapi_ticket
     * @return Ambigous|bool
     */
    private static function _getTicket(){
        $accessToken = self::getAccessToken();
        $url = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token={$accessToken}&type=jsapi";
        $ticket = Curl::callWebServer($url, '', 'GET');
        if(!isset($ticket['ticket'])){
            return Msg::returnErrMsg(MsgConstant::ERROR_GET_ACCESS_TOKEN, '获取ticket失败');
        }
    ...
    private static function _checkAccessToken(){
        ...
        $data = file_get_contents(ROOT.'access_token');
    ...
    private static function _checkTicket(){
        //获取access_token。是上面的获取方法获取到后存起来的。
        //$accessToken = YourDatabase::get('access_token');
        $data = file_get_contents(ROOT.'ticket');
        if(!empty($data)){
